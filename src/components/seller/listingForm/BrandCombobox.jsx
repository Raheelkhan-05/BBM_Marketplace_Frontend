...
export default function BrandCombobox({ value, notApplicable, image, onChange }) {
    ...
    if (notApplicable) {
        return (
            <div className="flex items-center justify-between rounded-xl border-2 px-3.5 py-2.5" style={{ borderColor: C.hairSoft, background: C.hairSoft }}>
                <span className="text-[13px] font-bold" style={{ color: C.muted }}>Brand: Not Applicable</span>
                <button type="button" onClick={() => onChange({ brandName: "", brandImage: null, brandNotApplicable: false })} className="text-[11.5px] font-bold underline" style={{ color: C.secondary }}>Change</button>
            </div>
        );
    }
    ...
    return (
        <div ref={boxRef} className="relative flex flex-col gap-1.5">
            <label className="text-[12px] font-bold" style={{ color: C.ink }}>Brand <span style={{ color: C.primary }}>*</span></label>
            <div className="flex items-center gap-2">
                <div className="relative flex-1">
                    ...
                    <input
                        ...
                        className="w-full rounded-xl border-2 py-2.5 pl-9 pr-3 text-[13.5px] font-semibold focus:outline-none focus:ring-4"
                ...
                {image ? (
                    ...
                    <label className="flex h-10 w-10 shrink-0 cursor-pointer items-center justify-center rounded-lg border-2 border-dashed" style={{ borderColor: C.hairSoft, color: C.muted }} title="Optional brand logo">
                        {uploading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <ImagePlus className="h-3.5 w-3.5" />}
                        <input type="file" accept="image/*" onChange={handleImage} className="hidden" disabled={uploading} />
                    </label>
                )}
            </div>
            <button type="button" onClick={() => onChange({ brandName: "", brandImage: null, brandNotApplicable: true })} className="w-fit text-[11px] font-bold underline" style={{ color: C.muted }}>
                This product has no brand
            </button>
            {open && (
                <div className="absolute left-0 right-0 top-[68px] z-30 max-h-56 overflow-y-auto rounded-xl border bg-white shadow-lg" style={{ borderColor: C.hair }}>
                    {items.map((it) => (
                        <button key={it.name} type="button" onClick={() => pick(it.name, it.image)} className="flex w-full items-center gap-2.5 border-b px-3.5 py-2.5 text-left last:border-b-0 hover:bg-black/[0.03]" style={{ borderColor: C.hairSoft }}>
                            ...
                            <span className="text-[13px] font-semibold" style={{ color: C.ink }}>{it.name}</span>
                        </button>
                    ))}
                    {items.length === 0 && <p className="px-3.5 py-3 text-center text-[12px] font-medium" style={{ color: C.muted }}>{q ? `No matches — "${q}" will be created as a new brand.` : "Start typing…"}</p>}
                </div>
            )}
        </div>
    );
