...
import { Search, Plus, Trash2, Loader2, ChevronDown, ChevronUp, AlertTriangle, TrendingDown, TrendingUp, X, Percent, IndianRupee } from "lucide-react";
...
function BuyerRow({ buyer, product, buyerDirectory, isEditing, onToggleEdit, draft, setDraft, onSave, onDelete, saving, deleting }) {
                ...
                <div className="flex flex-col gap-2.5 border-t px-3 py-3" style={{ borderColor: C.hairSoft }}>
                    ...
                    <div className="flex items-center gap-2">
                        <button type="button" onClick={onDelete} disabled={deleting}
                            className="flex items-center gap-1.5 rounded-lg border px-3 py-2 text-[11px] font-bold tracking-wide disabled:opacity-60" style={{ borderColor: C.hair, color: "#c71f11" }}>
                            {deleting ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Trash2 className="h-3.5 w-3.5" />} Remove
                        </button>
                        <button type="button" onClick={onSave}
                            disabled={saving || draft.canonicalPrice == null || violatesMinUnitPrice(draft.canonicalPrice, product.packSize, product.masterPackSize)}
                            className="ml-auto flex items-center gap-1.5 rounded-lg px-4 py-2 text-[11.5px] font-bold tracking-wide text-white disabled:opacity-50" style={{ background: C.secondary }}>
...
export default function ProductBuyerPricing({ submissionId }) {
    const { token } = useAuth();
    ...
    const save = async (buyerId) => {
        ...
        if (!draft?.canonicalPrice) return;
        setSavingId(buyerId);
        setError(null);
        const item = {
            submissionId,
        ...
        setSavingId(null);
        if (res?.rejected?.length) { setError(`That price would go below ₹${MIN_UNIT_PRICE}/unit — nothing was saved.`); return; }
        if (!res?.success && !res?.saved) { setError(res?.message || "Couldn't save this price."); return; }
        setEditingId(null);
        load(); // resync so effectivePrice/breakdown reflect exactly what was stored
    };
                ...
                <div className="flex flex-col gap-2">
                    ...
                    {buyers.map((b) => (
                        <BuyerRow
                            ...
                            onSave={() => save(b.buyerId)}
                            onDelete={() => remove(b.buyerId)}
                            saving={savingId === b.buyerId}
